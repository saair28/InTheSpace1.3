...
public class Cables2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (FindObjectOfType<AudioManager>().IsPlaying("Cables2") == true)
        {
            Debug.Log("sa");
        ...
        if (cronometro >= Stop)
        {
            FindObjectOfType<AudioManager>().Pause("Cables2");
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag == "Player" && cont <= 0 && verificar == false)
        {
            ...
            FindObjectOfType<AudioManager>().Play("Cables2");
